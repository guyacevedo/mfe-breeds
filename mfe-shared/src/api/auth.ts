...
import { User, RegisterRequest, LoginRequest, AuthResponse, LoadingState } from '../domain';
...
export class AuthService {
  ...
  login(credentials: LoginRequest): Observable<AuthResponse> {
    ...
    return this.makeRequest<AuthResponse>(`${API_BASE_URL}/users/login`, {
      ...
      tap(response => {
        console.log('Auth.login tap - full response:', response);
        console.log('Auth.login tap - response.user:', response.user);
        console.log('Auth.login tap - response.token:', response.token);
        this.setAuthData(response.user, response.token);
      }),
      finalize(() => {
        console.log('Auth.login finalize - setting loading to false');
  ...
  private setLoading(isLoading: boolean, error: string | null): void {
    console.log('Auth.setLoading called:', { isLoading, error });
  ...
  private setAuthData(user: User, token: string): void {
    ...
    console.log('Updating userStore with:', userWithToken);
